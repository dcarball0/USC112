    ...
    void Draw()
    {
        if (OsmReader.Instance.nodes != null)
        {
            ...
            for (int i = 0; i < intersections.Count; i++)
            {
                ...
                var fmh_39_58_638428095590508728 = Quaternion.identity; Handles.FreeMoveHandle(intersections[i], 1f, Vector2.zero, Handles.CylinderHandleCap);
            ...
            foreach (var u in intersection.edgesIntersection)
            {
                for (int i = 0; i < u.Length; i++)
                {
                    ...
                    var fmh_51_50_638428095590534771 = Quaternion.identity; Handles.FreeMoveHandle(u[i], 0.5f, Vector2.zero, Handles.CylinderHandleCap);
