...
public class PathEditor : Editor
{
    ...
    private void DebugDraw()
    {
        ...
        foreach (var n in nodes)
        {
            ...
            { var fmh_48_54_638428095590606875 = Quaternion.identity; Handles.FreeMoveHandle(n.Value - center, 2, Vector2.zero, Handles.CylinderHandleCap); }
        }
    }
    ...
    private void DrawOsm()
    {
        if (OsmReader.Instance.nodes != null)
        {
            ...
            foreach (var n in nodes)
            {
                var fmh_62_58_638428095590611024 = Quaternion.identity; Handles.FreeMoveHandle(n.Value - center, 1f, Vector2.zero, Handles.CylinderHandleCap);
    ...
    private void Draw()
    {
        for (int i = 0; i < path.pathSections.Count; i++)
        {
            ...
            for (int j = 1; j < section.Count; j++)
            {
                ...
                var fmh_97_69_638428095590615280 = Quaternion.identity; Vector2 newPos = Handles.FreeMoveHandle(section[j], .075f, Vector2.zero, Handles.CylinderHandleCap);
